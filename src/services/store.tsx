...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const [sync, setSync] = useState<SyncInfo>({ status: "connecting" });
  ...
  const loadSharedSpace = useCallback(async () => {
    const { data: shared, error } = await supabase
      ...
      .single();
    if (error || !shared) {
      setSync({ status: "error", error: error?.message || "Viagem não encontrada no Supabase." });
      return;
    }
    const revision = Number(shared.revision ?? 0);
    if (revision <= syncRevision.current && remoteSnapshot.current) return;
    ...
    if (Object.keys(remote as object).length > 0) {
      ...
      remoteSnapshot.current = JSON.stringify(next);
      setState(next);
    } else if (!remoteSnapshot.current) {
      const local = loadState();
      // O primeiro aparelho que abre o app grava o estado inicial na nuvem.
      const { data: initial, error: initialError } = await supabase
        .from("shared_link_spaces")
        .update({ data: local, revision: revision + 1, updated_at: new Date().toISOString() })
        .eq("id", shared.id)
        .eq("revision", revision)
        .select("revision")
        .maybeSingle();
      if (initialError) {
        setSync({ status: "error", error: initialError.message });
        return;
      }
      if (!initial) { void loadSharedSpace(); return; }
      syncRevision.current = Number(initial.revision);
      remoteSnapshot.current = JSON.stringify(local);
    }
    setSync({ status: "online", spaceId: shared.id, lastSync: Date.now() });
  }, []);

  useEffect(() => {
    if (!hydrated) return;
    void loadSharedSpace();
    const timer = setInterval(() => { void loadSharedSpace(); }, 3000);
    return () => clearInterval(timer);
  }, [hydrated, loadSharedSpace]);

  useEffect(() => {
    if (!hydrated || !sync.spaceId) return;
    const serialized = JSON.stringify(state);
    if (serialized === remoteSnapshot.current) return;
    if (syncTimer.current) clearTimeout(syncTimer.current);
    syncTimer.current = setTimeout(async () => {
      const nextRevision = syncRevision.current + 1;
      const { data: saved, error } = await supabase
        .from("shared_link_spaces")
        .update({ data: state, revision: nextRevision, updated_at: new Date().toISOString() })
        .eq("id", sync.spaceId)
        .eq("revision", syncRevision.current)
        .select("revision")
        .maybeSingle();
      if (error) {
        setSync((v) => ({ ...v, status: "error", error: error.message }));
      } else if (saved) {
        syncRevision.current = Number(saved.revision);
        remoteSnapshot.current = serialized;
        setSync((v) => ({ ...v, status: "online", lastSync: Date.now(), error: undefined }));
      } else {
        // Outro aparelho salvou primeiro; busca a versão mais recente.
        void loadSharedSpace();
      }
    }, 650);
    return () => { if (syncTimer.current) clearTimeout(syncTimer.current); };
  }, [state, hydrated, sync.spaceId, loadSharedSpace]);
