...
import { CloudChecklist } from "../CloudChecklist";
  ...
  return (
    <div className="pb-32">
      ...
      <section className="animate-fade-up mt-5 px-5">
        <p className="label">Nossa viagem na nuvem</p>
        <div className="card p-4 !shadow-none">
          <div className="flex items-center gap-3">
            <RowIcon icon={Cloud} />
            <div>
              <p className="text-[13.5px] font-bold">{sync.status === "online" ? "Conectado automaticamente" : sync.status === "connecting" ? "Conectando…" : "Sem conexão"}</p>
              <p className="mt-0.5 text-[11.5px] text-[#a6a0cc]">Os dois aparelhos com este app compartilham a mesma viagem, sem conta.</p>
            </div>
          </div>
          {sync.error && <p className="mt-3 text-[11px] text-red-300">{sync.error}</p>}
          {sync.spaceId && <CloudChecklist />}
        </div>
      </section>
